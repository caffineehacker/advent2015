...
fn main() {
    ...
    solve_part2(lights.clone(), width + 1);
}
...
fn solve_part2(mut lights: HashMap<(usize, usize), bool>, width: usize) {
    *lights.get_mut(&(0, 0)).unwrap() = true;
    *lights.get_mut(&(width - 1, 0)).unwrap() = true;
    *lights.get_mut(&(0, width - 1)).unwrap() = true;
    *lights.get_mut(&(width - 1, width - 1)).unwrap() = true;
    for _ in 0..100 {
        let mut new_lights = lights.clone();
        new_lights.iter_mut().for_each(|(pos, l)| {
            let mut neighbors_lit = 0;
